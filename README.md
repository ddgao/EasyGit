...
- **一键合并**：功能分支依次合并到 dev、test、main（顺序执行，确保每个环境合并成功后再进行下一个）
- 合并前自动 fetch 最新代码
- 冲突检测和提示，中断流程让用户手动解决
- 可配置是否自动 push
- 合并完成后自动切回原分支
...
**使用场景示例**：
1. **统一版本号**：为所有微服务打同一个版本（如 v1.2.0）
2. **独立版本号**：某些服务有特殊需求，可设置不同版本（如项目A: v1.2.0, 项目B: v1.2.0-P01）
3. **批量调整**：输入基础版本后，可单独调整个别仓库的后缀
...
右键项目目录 → EasyGit
    ...
    ├── 打 Tag...（打开对话框）
    ├── 快速打正常版本 Tag（v1.0.0）
    ├── 快速打临时需求 Tag（v1.0.0-P01）
    └── 快速打 Hotfix Tag（v1.0.0-H01）
```
...
| 配置项 | 默认值 | 说明 |
|--------|--------|------|
| Tag 版本号模式 | `^(R_\d+\.\d+\.\d+|v?\d+\.\d+\.\d+).*$` | 用于提取 Tag 基础版本号的正则表达式（第一个捕获组） |
